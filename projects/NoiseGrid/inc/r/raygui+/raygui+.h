...
struct rBounds 
{
    ...
    rBounds(float width, float height) : bounds({0, 0, width, height}) {}
    union {
        struct      { float left = 0, top = 0, width = 0, height = 0; };
...
struct rContainerBase : public rBounds {
    rContainerBase(std::vector<rBounds*>  *children, 
        ...
        : rBounds(bounds), padding(padding), children(*children), ranchor(ranchor), adjust(adjust), enabled(enabled), parent(parent) 
        {}
...
struct rHBox : public rContainerBase {
    rHBox(Rectangle              bounds, 
          ...
          rContainerBase*        parent = nullptr,
          bool                   enabled = true) 
        : rContainerBase(&children, anchor, bounds, padding, adjust, parent, enabled), spacing(spacing) 
        {}
    rHBox(std::vector<rBounds*>  children, 
          float                  spacing = 0, 
          rAnchor                anchor = ANCHOR_NONE, 
          rContainerAdjust       adjust = FIT_NONE, 
          Rectangle              bounds = {}, 
...
struct rVBox : public rContainerBase {
    rVBox(Rectangle              bounds, 
        ...
        : rContainerBase(&children, anchor, bounds, padding, adjust, parent, enabled), spacing(spacing) 
        {}
    rVBox(std::vector<rBounds*>  children, 
          float                  spacing = 0, 
          rAnchor                anchor = ANCHOR_NONE, 
          rContainerAdjust       adjust = FIT_NONE, 
          Rectangle              bounds = {}, 
...
struct rLine : public rBounds {
    ...
    rLine(float width, float height, const char *text) 
    : rBounds({0, 0, width, height}), text(text) {}
...
struct rLabel : public rBounds {
    ...
    : rBounds(bounds), text(text) 
    {}
    rLabel(float width, float height, const char *text) 
    : rBounds({0, 0, width, height}), text(text) 
    {}
    rLabel(const char *text) 
    : text(text) 
    {}
